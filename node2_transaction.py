...
class Blockchain:
    ...
    def verify_transaction_signature(self, sender_address, receiver_address, transaction):
        ...
        response = requests.post(f'http://{urlparse(receiver_address).netloc}/add_wallet', data=transactions,
                                 json=transaction)
...
def mine_block():
    ...
    blockchain.add_transaction(sender=node_address,receiver='Aditya', amount=10, ismined=True)
